/* return_map_rat_finder.c */

/* to compile using gcc type:
   gcc return_map_rat_finder.c -lgmp
...
/* It was used to compute a list of rationals 0 < q < 1 with denominator at
most 30 such that for all rational 0 < r < 1 with denominator at most 1000
rot(f_q,r) is rational as detailed in "A piecewise linear homeomorphim of the
circle which is periodic under renormalization" by James Belk, James Hyde,
and Justin Tatch Moore */
...
int main()
{
  ...
  bool report_rational=false,report_irrational=false,colored_output=true; /* These flags determine whether examples with rational or
  ...
  for(Q.n=3L;Q.n<=30L;Q.n++)
      ...
      if(gcd(Q.m,Q.n)==1)
	{
	  ...
	  if(rc==0)
	    {
	      printf("When q=%li/%li, rot(f_q,r) seems to be rational.\n",Q.m,Q.n);
