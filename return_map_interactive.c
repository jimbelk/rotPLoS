/* return_map_interactive.c */

/* to compile using gcc type:
   gcc return_map_interactive.c -lgmp
...
/* This code allows the user to interactively compute rot(f_q,r) when 
   0 < q,r < 1 are rational.  See the paper
...
bool PL_equal(struct point *f, struct point *g)
...
int main()
{
  struct fraction Q,R; /* This variable contains the current value of q for f_q,r */
  long i,j,k,l,m,n;
  ...
  char *str1,*str2,*r_str;
  bool repeat,rational,done=false;
  mpq_t q,r,s,t,tmp;
  ...
  while(!done)
    {
	 ...
	 and then allocates and initializes the space. */\
      printf("This program computes the rotation number of f_q,r\n");
      printf("Enter q in the format m/n:  ");
      scanf("%li/%li",&Q.m,&Q.n);
      if((Q.m*Q.n==0)||(Q.m<0))
	exit(0);
      printf("Enter r in the format m/n:  ");
      scanf("%li/%li",&R.m,&R.n);
      if((R.m*R.n==0)||(R.m <0)||(R.n <= R.m))
	exit(0);
      printf("\n");
      printf("The following are the PL homeomorphisms resulting from iterating f->f* starting with f_q,r\n\n");
      mpq_set_ui(q,Q.m,Q.n);
      mpq_canonicalize(q);
      mpq_set_ui(r,R.m,R.n);
      mpq_canonicalize(r);
      init_f_qr(q,r,f);    
      loc[0]=0; 
      theta[0]=0;
      repeat=false; /* repeat and rational are flags indicating either there has been a repeat in the
		       renormalization iteration f -> f* or else that a fixed point has been encountered,
		       indicating a rational rotation number. */
      rational=false;
      for(m=1;(m<D-1)&&(!repeat)&&(!rational);m++)
	{
      ...
      if(!rational)
	{
	  if(repeat)
	    {
	      r_str=mpq_get_str(NULL,10,r);
	      printf("When q=%li/%li r=%s, rot(f_q,r) has continued fraction expansion [0; ",Q.m,Q.n,r_str); 
	      free(r_str);
	      for(j=0;j < k;j++)
		{
		  printf("%li",theta[j]);
	      ...
	      for(j=k;j < m;j++)
		{
	      ...
	      printf("Here rot(f_q,r) is irrational and the periodic part of the continued fraction is in parentheses.\n\n");
	    }
	  else if(!repeat)
	    {
	      ...
	      for(j=0;j < m;j++)
		{
	      ...
	      printf("This count be an example in which rot(f_q,r) is not algebraic of degree <=2.\n");
	      printf("Please let Justin Moore (justin@math.cornell.edu) know of this example.\n\n");
	      reset();
	    }
	}
      else if(rational)
	{
	  cyan();
	  r_str=mpq_get_str(NULL,10,r);
	  printf("When q=%li/%li r=%s, rot(f_q,r) = ",Q.m,Q.n,r_str); 
	  free(r_str);
	  j=mpq_set_cf(s,theta); 
	  str1=mpq_get_str(NULL,10,s);
	  printf("%s \n",str1);
	  printf("Which has continued fraction expansion: [ 0; ");
	  for(j=0;j < m-1;j++)
	    {
	      printf("%li",theta[j]);
	      if(j < m-2)
	  ...
	  printf("Continued fraction had %li digits.\n\n",j);
	  free(str1);
	  reset();
	}
    }    
